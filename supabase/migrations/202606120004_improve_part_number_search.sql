...
create or replace function public.normalize_part_number_search(p_value text)
returns text
language sql
immutable
...
create index if not exists products_active_part_number_search_trgm_idx
    ...
    select
      ...
      + case
          when search_input.part_query is not null
            and public.normalize_part_number_search(coalesce(p.sku, '')) = search_input.part_query then 5
          when search_input.part_query is not null
            and public.normalize_part_number_search(coalesce(p.sku, '')) like search_input.part_query || '%' then 3
          when search_input.part_query is not null
            ...
            ) like '%' || search_input.part_query || '%' then 2
          else 0
        end as search_rank,
...
comment on function public.normalize_part_number_search(text)
is 'Normalizes SKU and part-number-like text by removing punctuation for reliable screen part number search.';

comment on index public.products_active_part_number_search_trgm_idx
is 'Supports fast part number search across SKU, title, compatibility, warranty, highlights, and search keywords.';
