...
-- 1) Replace the obsolete 'packed' order status with 'ready_for_delivery'.
--    Orders are now auto-shipped at creation, so 'packed' no longer exists in
--    the workflow. No rows currently use 'packed'.
