-- Stop checkout from saving a duplicate address row every time a customer
-- reuses an already-saved address. Only persist a new address when it differs
-- from every existing one for the user. A row counts as the "same" address only
-- when the full name, phone, and the whole address (line1/line2/city/state/
-- pincode) all match (trimmed, case-insensitive). So a different address, or the
-- same address under a different name or phone number, still saves as a new one.
