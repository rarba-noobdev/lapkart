...
as $$
      ...
      case
        when input.query_text is null then null
          ...
          public.normalize_part_number_search(
            regexp_replace(
              input.query_text,
              '\m(display|screen|lcd|led|laptop|replacement|panel)\M',
              ' ',
              'gi'
            )
          ),
  ...
  candidate_ids as (
    ...
    where search_input.part_query is not null
      ...
      and (p_in_stock is distinct from true or p.stock > 0)
      and (
        p.normalized_search_document like '%' || search_input.part_query || '%'
        or (
          search_input.strict_display_phrase
          and search_input.display_phrase_query is not null
          and p.normalized_search_document like '%' || search_input.display_phrase_query || '%'
        )
      )
  ),
  matched as (
    select
      ...
      + case
          ...
          when search_input.strict_display_phrase
            and search_input.display_phrase_query is not null
            and p.normalized_search_document like '%' || search_input.display_phrase_query || '%' then 3
          else 0
        end as search_rank,
...
is 'Public active-product search. SECURITY DEFINER is intentionally used to avoid RLS planning overhead; strict display phrases require normalized phrase matches to avoid unrelated technical-spec word combinations.';
