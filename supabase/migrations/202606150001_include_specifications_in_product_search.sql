...
create or replace function public.product_structured_search_text(
  p_title text,
  p_brand text,
  p_category text,
  p_description text,
...
as $$
  select
    ...
    coalesce(
      (
        select string_agg(spec.key || ' ' || spec.value, ' ')
  ...
  matched as (
    select
      ...
      case
        ...
        else ts_rank_cd(to_tsvector('english', search_document.search_text), search_input.web_query)
      end
      + case
      ...
      + case
          ...
          when search_input.part_query is not null
            and search_document.part_search_text = search_input.part_query then 6
          when search_input.part_query is not null
            and search_document.part_search_text like '%' || search_input.part_query || '%' then 4
          else 0
        end as search_rank,
    ...
    where p.status = 'active'
      ...
      and (
        ...
        or to_tsvector('english', search_document.search_text) @@ search_input.web_query
        or search_document.search_text ilike '%' || search_input.query_text || '%'
        or (
          search_input.part_query is not null
          and search_document.part_search_text like '%' || search_input.part_query || '%'
        )
      )
  )
  select
...
comment on function public.product_structured_search_text(text, text, text, text, text, text, text, text[], text[], jsonb)
is 'Builds the indexed catalog search corpus from product identity, fitment, keywords, highlights, and structured specifications.';

comment on function public.search_active_products(text, text, text, numeric, numeric, boolean, numeric, text, integer, integer)
is 'Indexed product catalog search using Postgres full-text, structured specifications, and normalized part-number matching.';
