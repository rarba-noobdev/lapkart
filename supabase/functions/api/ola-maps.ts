...
export async function autocompleteOlaPlaces(
  ...
  return (data.predictions ?? []).slice(0, 6).map((prediction) => {
    const predictionLocation = prediction.geometry?.location;
    return {
      placeId: prediction.place_id ?? "",
      description: prediction.description ?? "",
      mainText: prediction.structured_formatting?.main_text ?? prediction.name ?? "",
      secondaryText: prediction.structured_formatting?.secondary_text ?? "",
      latitude: typeof predictionLocation?.lat === "number" ? predictionLocation.lat : null,
      longitude: typeof predictionLocation?.lng === "number" ? predictionLocation.lng : null,
