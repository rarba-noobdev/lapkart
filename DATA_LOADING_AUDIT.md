...
| `src/routes/products/+page.server.ts` | `/products` | Search route did not pass a page/limit to the shared search helper. | Parses `page`, passes `limit: 96`, returns page metadata. | Current UI remains unchanged; Typesense still returns total, Supabase fallback is bounded. | Prevents loading more than 96 catalog cards per request. | Uses existing `depends('app:products')`. |
